import { useGameStore } from '@store/store';
import styles from '@styles/VolumeControl.module.css';
import { useState, useRef, useEffect } from 'react';

const VolumeControl = () => {
  const { isMuted, toggleMute, audioVolume, setVolume } = useGameStore(state => ({
    isMuted: state.isMuted,
    toggleMute: state.toggleMute,
    audioVolume: state.audioVolume,
    setVolume: state.setVolume
  }));

  const [isVolumeOpen, setIsVolumeOpen] = useState(false);
  const volumePanelRef = useRef<HTMLDivElement>(null);
  const volumeContainerRef = useRef<HTMLDivElement>(null);

  // Close volume panel when clicking outside
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        volumePanelRef.current && 
        volumeContainerRef.current && 
        !volumeContainerRef.current.contains(event.target as Node)
      ) {
        setIsVolumeOpen(false);
      }
    };

    document.addEventListener('mousedown', handleClickOutside);
    return () => {
      document.removeEventListener('mousedown', handleClickOutside);
    };
  }, []);

  const handleVolumeButtonClick = () => {
    setIsVolumeOpen(!isVolumeOpen);
  };

  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newVolume = parseInt(e.target.value, 10);
    setVolume(newVolume);
  };

  return (
    <div className={styles.volumeContainer} ref={volumeContainerRef}>
      <button 
        className={styles.volumeButton} 
        onClick={handleVolumeButtonClick}
        aria-label="Music volume control"
        title="Music volume control"
      >
        {isMuted ? (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
            <path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" />
            <path d="M0 0h24v24H0z" fill="none"/>
          </svg>
        ) : (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
            <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" />
            <path d="M0 0h24v24H0z" fill="none"/>
          </svg>
        )}
      </button>

      {isVolumeOpen && (
        <div className={styles.volumePanel} ref={volumePanelRef}>
          <div className={styles.volumeHeader}>
            <span>Volume</span>
            <button 
              className={styles.muteButton} 
              onClick={toggleMute}
              title={isMuted ? "Unmute" : "Mute"}
            >
              {isMuted ? "Unmute" : "Mute"}
            </button>
          </div>
          <div className={styles.volumeSliderContainer}>
            <input 
              type="range" 
              min="0" 
              max="100" 
              value={audioVolume} 
              onChange={handleVolumeChange}
              className={styles.volumeSlider}
              disabled={isMuted}
            />
            <span className={styles.volumeValue}>{audioVolume}%</span>
          </div>
        </div>
      )}
    </div>
  );
};

export default VolumeControl; 